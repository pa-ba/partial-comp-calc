...
module Calculations.Terminating.Stack.LambdaConcur where

open import Calculations.Stack.LambdaConcur
...
mutual
  ...
  exec' (suc j) (FORK c' c)  (s , e)               = do ch ← newChan
                                                        exec' j c' ([] , Num' ch ∷ e) ∥ʳ exec' j c (VAL (Num' ch) ∷ s , e)
...
mutual
  ...
  execBisim (SEND c) (VAL (Num' x) ∷ VAL (Num' x₁) ∷ s , e) (suc i) .(suc _) (s≤s le) = ~i>>-cong-r _ (execBisim c _ _ _ le)
  ...
  execBisim (RECEIVE c) (VAL (Num' x) ∷ s , e) (suc i) .(suc _) (s≤s le) = ~i>>=-cong-r _ (λ a → execBisim c _ _ _ le)
  execBisim (RECEIVE c) (VAL (Clo' x x₁) ∷ s , e) (suc i) .(suc _) (s≤s le) = ~irefl
  execBisim (RECEIVE c) (CLO x x₁ ∷ s , e) (suc i) .(suc _) (s≤s le) = ~irefl
  execBisim (FORK c' c) (s , e) (suc i) (suc j) (s≤s le) = ~i>>=-cong-r _ λ ch →
    ~i∥ʳ-cong (execBisim c' _ _ j (lemma' (csize c') (csize c) (fsize (s , e)) j le))
