...
-- the concurrent lambda calculus is indeed well-defined.

module Calculations.Terminating.Memory.LambdaConcur where

open import Calculations.Memory.LambdaConcur
...
-- leaves `l` unchanged (or, in the case of the forked thread of
-- `FORK`, empties it) and makes the code argument structurally
...
mutual
  ...
  exec' k (SEND r c)   (Num' n , e , l , m)  = do ch ← get r m >>= getNum'
                                                  send ch n
                                                  exec' k c (Num' n , e , l , m)
  exec' k (RECEIVE c)  (Num' ch , e , l , m) = do n ← receive ch
                                                  exec' k c (Num' n , e , l , m)
  exec' k (FORK c' c)  (a , e , l , m)       = do ch ← newChan
                                                  exec' k c' (Num' 0 , Num' ch ∷ e , [] , empty)
                                                    ∥ʳ exec' k c (Num' ch , e , l , m)
...
mutual
  ...
  execBisim (SEND r c) (Num' n , e , l , m) (suc i) k le =
    ~i>>=-cong-r (get r m >>= getNum') (λ ch → ~i>>-cong-r (send ch n) (execBisim c _ _ k le))
  execBisim (SEND r c) (Clo' c' e' , _ , l , m) (suc i) k le = ~irefl
  execBisim (RECEIVE c) (Num' ch , e , l , m) (suc i) k le =
    ~i>>=-cong-r (receive ch) (λ n → execBisim c _ _ k le)
  execBisim (RECEIVE c) (Clo' c' e' , _ , l , m) (suc i) k le = ~irefl
  execBisim (FORK c' c) (a , e , l , m) (suc i) k le =
    ~i>>=-cong-r newChan (λ ch → ~i∥ʳ-cong (execBisim c' _ _ k z≤n) (execBisim c _ _ k le))
