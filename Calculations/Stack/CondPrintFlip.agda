...
-- non-determinism, and a print effect.
------------------------------------------------------------------------

module Calculations.Stack.CondPrintFlip where
...
mutual
  ...
  exec (FLIP c) s = exec c (B true ∷ s) ⊕ exec c (B false ∷ s)
...
spec i Flip {s} {c} =
  (do v ← eval Flip
      exec c (v ∷ s))
  ≡⟨⟩
  (do v ← return (B true) ⊕ return (B false)
      exec c (v ∷ s))
  ≡⟨⟩ 
  (do v ← return (B true); exec c (v ∷ s)) ⊕
  (do v ← return (B false); exec c (v ∷ s))
  ≡⟨⟩ -- monad left unit law: 
  (exec c (B true ∷ s)) ⊕ (exec c (B false ∷ s))
   ≡⟨⟩ 
  (exec (FLIP c) s)
  ∎
...
eval-safe ⊢flip = spplus (spnow ⊢B) (spnow ⊢B)
