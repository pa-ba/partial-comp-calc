...
------------------------------------------------------------------------
-- Calculation for Rattus
------------------------------------------------------------------------

module Calculations.Stack.Rattus where
...
getInto : ∀ {i} → Value → Partial Value i
getInto (IntoV v) = return v
getInto _ = stuck

getUnit : ∀ {i} → Value → Partial ⊤ i
getUnit UnitV = return tt
getUnit _ = stuck

caseSum : ∀ {A i} → Value → (Value → Partial A i) → (Value → Partial A i) → Partial A i
caseSum (In1V v) k1 k2 = k1 v
caseSum (In2V v) k1 k2 = k2 v
caseSum _ _ _ = stuck
...
lookup l [] = stuck
lookup zero (x ∷ η) = return x
lookup (suc l) (x ∷ η) = lookup l η
...
mutual
  ...
  eval (Var x) e σ = do v ← lookup x e
                        return (v , σ)
  ...
  eval (Pr2 t) e σ = do v , σ' ← eval t e σ
                        v1 , v2 ← getPair v
                        return (v2 , σ')
  eval (In1 t) e σ = do v , σ' ← eval t e σ
                        return (In1V v , σ')
  eval (In2 t) e σ = do v , σ' ← eval t e σ
                        return (In2V v , σ')
  eval (Case t t1 t2) e σ = do v , σ' ← eval t e σ
                               caseSum v (λ vl → eval t1 (vl ∷ e) σ') (λ vr → eval t2 (vr ∷ e) σ')
  eval (Into t) e σ = do v , σ' ← eval t e σ
                         return (IntoV v , σ')
  eval (Out t) e σ = do v , σ' ← eval t e σ
                        v' ← getInto v
                        return (v' , σ')
  eval Unit e σ = return (UnitV , σ)
...
-- separately in the `Terminating.Stack.Rattus` module.
...
mutual
  ...
  exec (IN1 c) (VAL v ∷ s , e , σ) = exec c (VAL (In1' v) ∷ s , e , σ)
  exec (IN2 c) (VAL v ∷ s , e , σ) = exec c (VAL (In2' v) ∷ s , e , σ)  
  exec (LOOKUP n c) (s , e , σ) = do v <- lookup n e
                                     exec c (VAL v ∷ s , e , σ)
  ...
  exec (CASE c1 c2) (VAL (In1' v) ∷ s , e , σ) = exec c1 (s , v ∷ e , σ)
  exec (CASE c1 c2) (VAL (In2' v) ∷ s , e , σ) = exec c2 (s , v ∷ e , σ)  
  exec (INTO c) (VAL v ∷ s , e , σ) = exec c (VAL (Into' v) ∷ s , e , σ)  
  exec (OUT c) (VAL (Into' v) ∷ s , e , σ) = exec c (VAL v ∷ s , e , σ)
  exec (UNIT c) (s , e , σ) = exec c (VAL Unit' ∷ s , e , σ)
  exec _ _ = stuck
...
comp (In1 x) c = comp x (IN1 c)
comp (In2 x) c = comp x (IN2 c)
comp (Into x) c = comp x (INTO c)
comp (Out x) c = comp x (OUT c)
...
mutual
  ...
  conv (In1V x) = In1' (conv x)
  conv (In2V x) = In2' (conv x)  
  conv (IntoV x) = Into' (conv x)
  conv UnitV = Unit'
...
lookup-convH : ∀ {i A} n e → (f : Code × Env' → Partial A ∞) →
  (lookup n e >>= (λ (t , e) → f (comp t RET , convE e) )) ~[ i ] (lookup n (convH e) >>= f)
...
spec i (Add x y) {s} {c} {e} {σ} =
  ...
  ⊥~⟨ ⊥~i>>=-cong-r (eval x e σ) (λ { (NumV _ , _) → ⊥~irefl ; (CloV _ _ , _) → ⊥~istuck ; (LocV _ , _) → ⊥~istuck ;
      (BoxV _ _ , _) → ⊥~istuck ; (PairV _ _ , _) → ⊥~istuck ; (In1V _ , _) → ⊥~istuck ; (In2V _ , _) → ⊥~istuck ;
       (IntoV _ , _) → ⊥~istuck ; (UnitV , _) → ⊥~istuck }) ⟩
  (do v1 , σ1 ← eval x e σ
      v2 , σ2 ← eval y e σ1
      n2 ← getNum v2
  ...
  ⊥~⟨ ⊥~i>>=-cong-r (eval x e σ) (λ (v1 , σ1) → 
      ...
      (BoxV _ _ , _) → ⊥~istuck ; (PairV _ _ , _) → ⊥~istuck ; (In1V _ , _) → ⊥~istuck ;
      (In2V _ , _) → ⊥~istuck ; (IntoV _ , _) → ⊥~istuck ; (UnitV , _) → ⊥~istuck })) ⟩
  (do v1 , σ1 ← eval x e σ
      v2 , σ2 ← eval y e σ1
...
spec (suc i) (Adv x) {s} {c} {e} { ⟨ ηN ✓ ηL ⟩ } =
  ...
  ⊥~⟨ ⊥~i>>=-cong-r (eval x  e ⟨ ηN ⟩)
    (λ { (NumV x , _) → ⊥~istuck ; (CloV x x₁ , _) → ⊥~istuck ; (LocV x , _) → ⊥~irefl ; (BoxV x x₁ , _) → ⊥~istuck ;
      (PairV v v₁ , _) → ⊥~istuck ; (In1V v , _) → ⊥~istuck ; (In2V v , _) → ⊥~istuck ; (IntoV _ , _) → ⊥~istuck ; (UnitV , _) → ⊥~istuck }) ⟩
  (do v , σ ← eval x  e ⟨ ηN ⟩
...
spec (suc i) (Unbox x) {s} {c} {e} {σ} =
  ...
  ⊥~⟨ ((⊥~i>>=-cong-r (eval x e σ)
    λ { (NumV x , σ') → ⊥~istuck ; (CloV _ _ , σ') → ⊥~istuck ; (LocV x , σ') → ⊥~istuck ; (BoxV _ _ , σ') → ⊥~irefl ;
      (PairV _ _ , σ') → ⊥~istuck ; (In1V _ , σ') → ⊥~istuck ; (In2V _ , σ') → ⊥~istuck ; (IntoV _ , _) → ⊥~istuck ; (UnitV , _) → ⊥~istuck })) ⟩
  (do v , σ' ← eval x e σ
...
spec i@(suc j) (App x y) {s} {c} {e} {σ} =
  ...
  ⊥~⟨ (⊥~i>>=-cong-r (eval x e σ)
    λ { (NumV x , σ1) → ⊥~istuck ; (CloV x x₁ , σ1) → ⊥~irefl ; (LocV x , σ1) → ⊥~istuck ; (BoxV x x₁ , σ1) → ⊥~istuck ;
      (PairV v1 v2 , σ1) → ⊥~istuck ; (In1V v1 , σ1) → ⊥~istuck ; (In2V v1 , σ1) → ⊥~istuck ; (IntoV _ , _) → ⊥~istuck  ; (UnitV , _) → ⊥~istuck }) ⟩
  (do v1 , σ1 ← eval x e σ
      v2 , σ2 ← eval y e σ1
...
spec i (Pr1 x) {s} {c} {e} {σ} =
  ...
  ⊥~⟨ (~i>>=-cong-r (eval x e σ)
    λ { (NumV x , σ') → ⊥~istuck ; (CloV x x₁ , σ') → ⊥~istuck ; (LocV x , σ') → ⊥~istuck ; (BoxV x x₁ , σ') → ⊥~istuck ;
      (PairV v v₁ , σ') → ⊥~irefl ; (In1V v , σ') → ⊥~istuck ; (In2V v , σ') → ⊥~istuck ; (IntoV _ , _) → ⊥~istuck ; (UnitV , _) → ⊥~istuck }) ⟩
  (do v , σ' ← eval x e σ
...
spec i (In1 x) {s} {c} {e} {σ} =
  (do v , σ' ← eval (In1 x) e σ
      exec c (VAL (conv v) ∷ s , convE e , convS σ'))
  ~⟨ ~i>>=-assoc (eval x e σ) ⟩
  (do v , σ' ← eval x e σ
      exec c (VAL (In1' (conv v)) ∷ s , convE e , convS σ'))
  ≡⟨⟩
  (do v , σ' ← eval x e σ
      exec (IN1 c) (VAL (conv v) ∷ s , convE e , convS σ'))
  ⊥~⟨ spec i x ⟩
  (do exec (comp x (IN1 c)) (s , convE e , convS σ))
  ∎
  
spec i (In2 x) {s} {c} {e} {σ} =
  (do v , σ' ← eval (In2 x) e σ
      exec c (VAL (conv v) ∷ s , convE e , convS σ'))
  ~⟨ ~i>>=-assoc (eval x e σ) ⟩
  (do v , σ' ← eval x e σ
      exec c (VAL (In2' (conv v)) ∷ s , convE e , convS σ'))
  ≡⟨⟩
  (do v , σ' ← eval x e σ
      exec (IN2 c) (VAL (conv v) ∷ s , convE e , convS σ'))
  ⊥~⟨ spec i x ⟩
  (do exec (comp x (IN2 c)) (s , convE e , convS σ))
  ∎
...
spec i (Case x x1 x2) {s} {c} {e} {σ} =
  ...
  ⊥~⟨ (⊥~i>>=-cong-r (eval x e σ)
    λ { (NumV x , σ') → ⊥~istuck ; (CloV x x₁ , σ') → ⊥~istuck ; (LocV x , σ') → ⊥~istuck ; (BoxV x x₁ , σ') → ⊥~istuck ;
      (PairV v v₁ , σ') → ⊥~istuck ; (In1V v , σ') → ~irefl ; (In2V v , σ') → ~irefl  ; (IntoV _ , _) → ⊥~istuck; (UnitV , _) → ⊥~istuck }) ⟩
  (do v , σ' ← eval x e σ
      caseSum v
  ...
  ⊥~⟨ ⊥~i>>=-cong-r (eval x e σ)
    ...
    (PairV v v₁ , σ) → ⊥~irefl ; (In1V v , σ) → spec i x1 ; (In2V v , σ) → spec i x2 ; (IntoV _ , _) → ⊥~istuck ; (UnitV , _) → ⊥~irefl}) ⟩
  (do v , σ' ← eval x e σ
      caseSum v
  ...
  ~⟨ ~i>>=-cong-r (eval x e σ)
    (λ { (NumV x , σ) → ~irefl ; (CloV x x₁ , σ) → ~irefl ; (LocV x , σ) → ~irefl ; (BoxV x x₁ , σ) → ~irefl ;
    (PairV v v₁ , σ) → ~irefl ; (In1V v , σ) → ~irefl ; (In2V v , σ) → ~irefl ; (IntoV _ , _) → ~irefl ; (UnitV , _) → ~irefl }) ⟩
  (do v , σ' ← eval x e σ
...
spec i (Into x) {s} {c} {e} {σ} =
  (do v , σ' ← eval (Into x) e σ
      exec c (VAL (conv v) ∷ s , convE e , convS σ'))
  ~⟨ ~i>>=-assoc (eval x e σ) ⟩
  (do v , σ' ← eval x e σ
      exec c (VAL (Into' (conv v)) ∷ s , convE e , convS σ'))
  ≡⟨⟩
  (do v , σ' ← eval x e σ
      exec (INTO c) (VAL (conv v) ∷ s , convE e , convS σ'))
  ⊥~⟨ spec i x ⟩
  (do exec (comp x (INTO c)) (s , convE e , convS σ))
  ∎

spec i (Out x) {s} {c} {e} {σ} =
  (do v , σ' ← eval (Out x) e σ
      exec c (VAL (conv v) ∷ s , convE e , convS σ'))
  ≡⟨⟩
  (do v , σ' ← do v , σ' ← eval x e σ
                  v' ← getInto v
  ...
  ⊥~⟨ (~i>>=-cong-r (eval x e σ)
    λ { (NumV x , σ') → ⊥~istuck ; (CloV x x₁ , σ') → ⊥~istuck ; (LocV x , σ') → ⊥~istuck ; (BoxV x x₁ , σ') → ⊥~istuck ;
      (PairV v v₁ , σ') → ⊥~istuck ; (In1V v , σ') → ⊥~istuck ; (In2V v , σ') → ⊥~istuck; (IntoV v , σ') → ⊥~irefl ; (UnitV , σ') → ⊥~istuck }) ⟩
  (do v , σ' ← eval x e σ
      exec (OUT c) (VAL (conv v) ∷ s , convE e , convS σ'))
  ⊥~⟨ spec i x ⟩
  (exec (comp x (OUT c)) (s , convE e , convS σ))
  ∎

spec i Unit {s} {c} {e} {σ} =
  (do v , σ' ← eval Unit e σ
      exec c (VAL (conv v) ∷ s , convE e , convS σ'))
  ≡⟨⟩
  (exec c (VAL Unit' ∷ s , convE e , convS σ))
  ≡⟨⟩
  (do exec (UNIT c) (s , convE e , convS σ))
  ∎
...
spec' : DNE → ∀ s c e σ x →
  ...
  ⊥~
  (exec (comp x c) (s , convE e , convS σ))
spec' dne s c e σ x =  ⊥~i-⊥~ dne  (λ i → spec i x)
...
specCompile : DNE → ∀ s σ x →
  (do v , σ' ← eval x [] σ
      return (VAL (conv v) ∷ s , [] , convS σ'))
  ⊥~
  (exec (compile x) (s , [] , convS σ))
specCompile dne s σ x = spec' dne s HALT [] σ x