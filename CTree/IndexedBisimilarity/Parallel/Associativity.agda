...
~i∥⃗-assoc : ∀ {i A B C E L} {{_ : Concurrent E}} (p : CTree E A ∞) (q : CTree E B ∞) (r : CTree E C ∞)
  → (p ∥⃗ q) ∥⃗ r ~ L [ i ] p ∥⃗ (q ∥⃗ r)
~i∥⃗-assoc p q r = prf p q r (<×⇐⁺×⇐⁺×⇐⁺-wf _) where
  ...
  prf {suc i} {L = L} p q r (acc rec) = ~istep left right where
    left : lsafe L ((p ∥⃗ q ∥⃗ r) ↑) → ∀ {l p' } → ((p ∥⃗ q ∥⃗ r) ↑) [ l ]⇒ p' → ∃[ q' ] ((p ∥⃗ (q ∥⃗ r)) ↑) [ l ]⇒ q' × p' ~̂ L [ lsuc l i ] q'
    left _ {l} tr with ∥⃗-step (p ∥⃗ q) r tr
