...
∥-lsafe-l : ∀ {E L A B} {{_ : Concurrent E}} {p : CTree E A ∞} {q : CTree E B ∞} → lsafe L ((p ∥ q) ↑) → lsafe L (p ↑)
∥-lsafe-l ls {l = ⟨ ε x ⟩} tr with ls (∥-stepLeft (LS retFreeε tr))
...
∥-lsafe-r : ∀ {E L A B} {{_ : Concurrent E}} {p : CTree E A ∞} {q : CTree E B ∞} → lsafe L ((p ∥ q) ↑) → lsafe L (q ↑)
∥-lsafe-r ls {l = ⟨ ε x ⟩} tr with ls (∥-stepRight (RS retFreeε tr))
... | isEffPredε .x x₁ = isEffPredε x x₁
∥-lsafe-r ls {l = ⟨ ι x ⟩} tr = isEffPredι x
∥-lsafe-r ls {l = ⟨ ρ x ⟩} tr = isEffPredρ x
